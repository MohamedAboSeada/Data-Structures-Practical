...
public:
    ...
    */
    ...
    // delete element before element in the list
    void delete_before(Any after_node_data){
        if(!(is_empty())){
            if(search(after_node_data)){
                Node<Any>* crr = head;
                while(crr->Data != after_node_data){
                    crr = crr->get_next();
                }
                if(crr->get_prev() != nullptr){
                    if(crr->get_prev()->get_prev() != nullptr){
                        Node<Any>* temp = crr->get_prev();
                        crr->set_prev(temp->get_prev());
                        temp->get_prev()->set_next(crr);
                        delete temp;
                        length--;
                    }else{
                        delete_start();
                    ...
                    cout<<"There aren't nodes before this node !\n";
                }
            }else{
                cout<<"element "<<after_node_data<<" not found !\n";
            }
